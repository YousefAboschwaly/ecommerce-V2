...
export default function Navbar() {
  ...
  return (
    <header className="sticky top-0 z-50 w-full border-b bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60">
      ...
      {userToken &&   <nav className="hidden md:flex items-center space-x-6 mx-auto">
          {menuItems.map((item) => (
            <AnimatedLink
              key={item.title}
              to={item.href}
        ...
        <div className={`flex items-center ml-auto  ${userToken&&' md:ml-0'}  gap-4 `}>
         ...
         {userToken && <div className="hidden md:flex items-center gap-4">
          ...
          {!userToken &&           <div className="hidden md:flex items-center gap-2">
              ...
              <SheetContent side="left" className="w-[300px] sm:w-[400px]">
                <nav className="flex flex-col  h-full">
                  <AnimatePresence>
                    {isOpen && (
                      <>
                        {userToken &&menuItems.map((item, index) => (
                          <motion.div
                            key={item.title}
...
{                 userToken && <motion.div
                        ...
                        >
                          <Link to="/wishlist" className="text-muted-foreground hover:text-primary" onClick={() => setIsOpen(false)}>
                            <Heart className="h-5 w-5" />
                          </Link>
...
{   userToken &&       <DropdownMenu>
            ...
            <DropdownMenuContent align="end">
              <DropdownMenuItem  className=" p-0">
                <Link to="/login" className="flex w-full p-1" onClick={logout}>
                  Logout
                </Link>
              </DropdownMenuItem>
            </DropdownMenuContent>
